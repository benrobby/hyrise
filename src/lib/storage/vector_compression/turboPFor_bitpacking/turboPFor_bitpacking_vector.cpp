...
TurboPForBitpackingVector::TurboPForBitpackingVector(pmr_vector<uint8_t>&& data, size_t size, uint8_t b) : _data{std::move(data)}, _size{size}, _b{b} {}
...
std::unique_ptr<const BaseCompressedVector> TurboPForBitpackingVector::on_copy_using_allocator(
  ...
  pmr_vector<uint8_t> data_copy(_data, alloc);
