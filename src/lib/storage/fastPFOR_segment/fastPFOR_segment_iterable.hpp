...
// todo: I just copied run_length_segment_iterable. We need to actually implement the iterable.
//  Also, all refs to end_positions are invalid because i removed the field.
template <typename T>
class FastPFORSegmentIterable : public PointAccessibleSegmentIterable<FastPFORSegmentIterable<T>> {
public:
    using ValueType = T;

    explicit FastPFORSegmentIterable(const FastPFORSegment<T>& segment) : _segment{segment} {}
    ...
    void _on_with_iterators(const std::shared_ptr<PosListType>& position_filter, const Functor& functor) const {
      ...
      auto begin =
              PointAccessIterator<PosListIteratorType>{_segment.encoded_values(), &_segment.null_values(), _segment.codec_id(), _segment.size(), position_filter->cbegin(), position_filter->cbegin()};
      auto end =
              PointAccessIterator<PosListIteratorType>{_segment.encoded_values(), &_segment.null_values(), _segment.codec_id(), _segment.size(), position_filter->cbegin(), position_filter->cend()};
      functor(begin, end);
    }
...
private:
    const FastPFORSegment<T>& _segment;

private:
    class Iterator : public AbstractSegmentIterator<Iterator, SegmentPosition<T>> {
    public:
        using ValueType = T;
        using IterableType = FastPFORSegmentIterable<T>;
